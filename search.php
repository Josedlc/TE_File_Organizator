...
$mysidesm = mysqli_real_escape_string($db, $_POST["id_esm"]);
$mysstr = mysqli_real_escape_string($db, $_POST["str"]);
$myswc = mysqli_real_escape_string($db, $_POST["wc"]);


$insertar = "SELECT * FROM estudio WHERE id_esm='$mysidesm' OR tipo='$mystipo' OR srecord='$mysstr' OR wc='$myswc'"; 
...
$codigo .='<th>'."Requiere".'</th>';
/*$codigo .='<th>'."Ing Manufactura".'</th>';
$codigo .='<th>'."Planta".'</th>';
$codigo .='<th>'."WC".'</th>';
$codigo .='<th>'."Estacion".'</th>';
$codigo .='<th>'."Parte".'</th>';
$codigo .='<th>'."Caracteristica".'</th>';
$codigo .='<th>'."SAP".'</th>';
...
$codigo .= '<td>'.utf8_encode($fila["id_esm"]).'</td>';
$codigo .= '<td>'.utf8_encode($fila["srecord"]).'</td>';
$codigo .= '<td>'.utf8_encode($fila["fechaalt"]) .'</td>';
$codigo .= '<td>'.utf8_encode($fila["requiere"]) .'</td>';
/*$codigo .= '<td>'.utf8_encode($fila["manufactura"]).'</td>';
$codigo .= '<td>'.utf8_encode($fila["planta"]).'</td>';
$codigo .= '<td>'.utf8_encode($fila["wc"]).'</td>';
$codigo .= '<td>'.utf8_encode($fila["estacion"]).'</td>';
$codigo .= '<td>'.utf8_encode($fila["parte"]).'</td>';
$codigo .= '<td>'.utf8_encode($fila["caracteristica"]).'</td>';
